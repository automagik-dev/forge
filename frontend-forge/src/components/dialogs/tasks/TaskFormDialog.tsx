...
import { forgeApi } from '@/lib/forgeApi';
  ...
  ({
    ...
    useEffect(() => {
      ...
      forgeApi
        ...
        .catch((err) => {
          console.warn('Failed to load forge branch template', err);
        });
    ...
    const syncBranchTemplate = useCallback(
      async (taskId: string, templateValue: string) => {
        const trimmed = templateValue.trim();
        try {
          if (trimmed) {
            await forgeApi.upsertBranchTemplate(taskId, trimmed);
          } else {
            await forgeApi.clearBranchTemplate(taskId);
          }
        } catch (err) {
          console.error('Failed to sync forge branch template', err);
        }
      },
      []
    );
          ...
          updateTask.mutate(
            ...
            },
            {
              onSuccess: (updatedTask) => {
                syncBranchTemplate(updatedTask.id, branchTemplate);
                modal.hide();
              },
            }
          );
          ...
          createTask.mutate(
            ...
            },
            {
              onSuccess: (createdTask) => {
                syncBranchTemplate(createdTask.id, branchTemplate);
                modal.hide();
              },
            }
          );
