...
echo "🔨 Building forge frontend (if exists)..."
if [ -d "frontend-forge" ]; then
    (cd frontend-forge && npm install && npm run build) || echo "⚠️  Frontend-forge build failed, continuing..."
fi
...
# Copy and zip the main binary (forge-app)
