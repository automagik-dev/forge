...
export function Breadcrumb() {
  ...
  const effectiveBaseBranch = useMemo(() => {
    // Don't calculate while branches are still loading
    if (branchesLoading) return null;

    // If error loading branches, return null (will show error state)
  ...
  }, [defaultBranch, branches, branchesLoading, branchesError]);
  ...
  // Pass undefined instead of null to satisfy hook typing
  ...
  const getBreadcrumbs = () => {
    ...
    if (projectId && project) {
      ...
      if (attempt) {
        ...
        // Only show if branches have loaded and effectiveBaseBranch is available
        ...
        // Show loading state when branches are loading (board view only)
        if (!taskId && branchesLoading) {
          crumbs.push({
            label: 'Loading...',
            path: location.pathname,
            type: 'board-base-branch',
        ...
        // Show error state when branches failed to load (board view only)
        if (!taskId && branchesError && !branchesLoading) {
          crumbs.push({
            ...
            <li key={`${crumb.type}-${crumb.path}-${index}`} className={`flex items-center gap-1 ${isParentTask ? 'hidden lg:flex' : ''}`}>
              ...
              {isCurrentProject ? (
                ...
                <div className="flex items-center gap-1">
                  ...
                  {isBoardBaseBranch && !branchesLoading && !branchesError && (
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
