...
export class McpConfigStrategyGeneral {
  static createFullConfig(cfg: McpConfig): ConfigObject {
    // create a template with servers filled in at cfg.servers
    const fullConfig = JSON.parse(JSON.stringify(cfg.template)) as ConfigObject;
    let current: ConfigObject = fullConfig;
  ...
  static validateFullConfig(
    ...
    full_config: ConfigObject
  ): void {
    // Validate using the schema path
    let current: unknown = full_config;
  ...
  static extractServersForApi(
    ...
    return current as ConfigObject;
  }

  static addPreconfiguredToConfig(
    mcp_config: McpConfig,
    existingConfig: ConfigObject,
    serverKey: string
  ): ConfigObject {
    const preconf = mcp_config.preconfigured as ConfigObject;
    if (!preconf || typeof preconf !== 'object' || !(serverKey in preconf)) {
      throw new Error(`Unknown preconfigured server '${serverKey}'`);
    }

    const updated = JSON.parse(JSON.stringify(existingConfig || {})) as ConfigObject;
    let current: ConfigObject = updated;
    ...
    (current[lastKey] as ConfigObject)[serverKey] = preconf[serverKey];
