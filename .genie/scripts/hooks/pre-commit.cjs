...
function run(script) {
  ...
  // Skip if script doesn't exist (worktree may not have .genie/scripts yet)
  if (!fs.existsSync(p)) {
    console.log(`⚠️  Skipping ${script.replace('.cjs', '')} (not found in worktree)`);
    return 0;
  }
...
function main() {
  ...
  const secretsCheckCode = timeExecution('Secret detection', () => {
    if (!fs.existsSync(checkSecretsPath)) {
      console.log('⚠️  Skipping check-secrets (not found in worktree)');
      return 0;
    }
  ...
  const pathsCheckCode = timeExecution('Path validation', () => {
    if (!fs.existsSync(validatePathsPath)) {
      console.log('⚠️  Skipping validate-paths (not found in worktree)');
      return 0;
    }
  ...
  const worktreeCheckCode = timeExecution('Worktree validation', () => {
    if (!fs.existsSync(worktreeCheckPath)) {
      console.log('⚠️  Skipping prevent-worktree-access (not found in worktree)');
      return 0;
    }
  ...
  try {
    const countTokensPath = path.join(gitRoot, '.genie', 'scripts', 'token-efficiency', 'count-tokens.cjs');
    timeExecution('Token counting', () => {
      if (!fs.existsSync(countTokensPath)) {
        console.log('⚠️  Skipping count-tokens (not found in worktree)');
        return 0;
      }
      spawnSync('node', [countTokensPath], { stdio: 'inherit' });
      return 0;
    });
  } catch (e) {
  ...
  try {
    const qualityGatePath = path.join(gitRoot, '.genie', 'scripts', 'token-efficiency', 'quality-gate.cjs');
    timeExecution('Quality gate check', () => {
      if (!fs.existsSync(qualityGatePath)) {
        console.log('⚠️  Skipping quality-gate (not found in worktree)');
        return 0;
      }
      spawnSync('node', [qualityGatePath], { stdio: 'inherit' });
      return 0;
    });
  } catch (e) {
