...
✅ Evidence of dry-runs or log verification captured in the Done Report
...
4. [Reporting]
   - Publish Done Report with files touched, validation evidence, safeguards, and follow-ups
...
```json
{
  ...
  "action": {
    "type": "confirm",
    "message": "Running cargo test --workspace can take time. Continue?",
    "fallback": "Abort if not approved"
  },
  "guards": {
    "allow_env": ["RUST_BACKTRACE"],
...
- Run targeted checks or inspect logs; record outputs in the Done Report.
- Save the report at `.genie/reports/done-forge-hooks-<slug>-<YYYYMMDDHHmm>.md` (UTC).
- Done Report must include files touched, validation steps, security notes, rollback instructions, and outstanding risks.
- Chat reply format:
  1. Numbered summary (discovery highlights, configuration actions, validation results).
  2. `Done Report: @.genie/reports/done-forge-hooks-<slug>-<YYYYMMDDHHmm>.md`.
