...
### Session Management
- Pick a stable session id (e.g., `wish-discovery-20250304`) and reuse it so `/agent` responses stay threaded.
- Log each response summary in the wish discovery section or Death Testament using short prompts to conserve tokens.
- Continue a dialogue with `/agent --session <same-id> --prompt "<follow-up>"`; record the delta and any decisions taken.
- For parallel viewpoints, start additional session ids (e.g., `wish-discovery-20250304-b`) and reconcile both outputs before finalizing.
- Track every session id and where its transcript/summary is stored for auditing.
