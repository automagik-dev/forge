...
impl TaskAttempt {
    ...
    pub async fn create(
        ...
        let template = forge_branch_templates::get_branch_template(pool, task_id)
            .await
            .map_err(TaskAttemptError::Database)?;

        let branch_name = forge_branch_templates::generate_branch_name(template, &task.title, &attempt_id);
