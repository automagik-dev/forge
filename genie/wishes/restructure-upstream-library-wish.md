...
2. [Implementation] Feature extraction and composition
   ...
   - Extract Omni, branch templates, config v7, and supporting migrations/services into the extension layer.
...
## Status – 2025-09-22
- `forge-app` now boots the upstream `DeploymentImpl`, runs Forge migrations, and serves the upstream API under `/api` while layering Forge overrides for task creation and task-attempt bootstrap.
- CLI packaging bundles `automagik-forge`, both frontend builds, and seeded config/SQLite files under `bloop/vibe-kanban/`; the launcher sets `FORGE_BUNDLE_PATH`, `XDG_DATA_HOME`, `APPDATA`, and `HOME` so upstream utilities resolve paths inside the extracted archive.
- Frontends: `/` serves `frontend-forge/dist`, `/legacy` proxies the upstream build from the submodule, and environment overrides (`FORGE_FRONTEND_DIST`, `FORGE_LEGACY_FRONTEND_DIST`) still work for local overrides.
- SQLx caches were regenerated against a fresh SQLite database (`target/sqlx-dev.sqlite`) seeded with both upstream and forge migrations; run `cargo sqlx prepare --workspace -- --all-targets` whenever queries change.
- Regression harness (`./scripts/run-forge-regression.sh`) now curls the Forge endpoints, rebuilds the CLI bundle, and records outputs under `docs/regression/latest/`.
...
- **Task 3**: `pnpm run lint`; `pnpm run build`; `cargo test --workspace` (or targeted crates); `cargo run -p forge-app` followed by `curl` smoke checks for `/health`, `/legacy`.
...
- Extract Omni, branch template, config v7, into extension crates.
- Create auxiliary tables & migrations with idempotent guards for branch templates, Omni settings, and config snapshots.
- Keep Genie automation as documentation-only; no HTTP endpoints or database fields are introduced for Genie.
...
- Implement regression harness back-end checks:
  ...
  - Call Omni webhook simulation, branch template CRUD, config read/write
...
Capture top offenders (e.g., omni services, branch template models) and reference them in task notes so extraction progress is measurable.
...
#[derive(RustEmbed)]
#[folder = "../upstream/upstream/frontend/dist"]
...
-- Optional: migrate Omni JSON blobs if present in upstream tables
INSERT OR REPLACE INTO forge_task_extensions (task_id, omni_settings)
SELECT id,
       omni_settings
FROM tasks
...
- [ ] Catalogue Genie/Claude documentation (no runtime extraction)
...
- **26 fork-only files** (documentation assets such as Genie wishes)
